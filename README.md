...
**最終更新**: 2025年6月12日 | **実装状況**: 全機能完了 + GitHub Actions CI/CD ✅
...
- ✅ **Git連携自動デプロイ** - GitHub Actions + clasp CI/CD（設定完了済み）
...
### 5. **Git自動デプロイ設定（✅ 設定完了済み）**

GitHub Actions による自動デプロイが設定済みです。以下のように動作します：

- **developブランチ** → 開発用GASプロジェクトに自動デプロイ
- **mainブランチ** → 本番用GASプロジェクトに自動デプロイ
...
2. `gas-version/**`のファイル変更でGitHub Actions自動実行
3. 開発用GASプロジェクトに自動デプロイ
4. テスト完了後、`main`にマージして本番デプロイ
...
### 🔄 **自動化**
- Git pushでの自動デプロイ（develop/main環境分離）
- スケジュール投稿の自動実行
